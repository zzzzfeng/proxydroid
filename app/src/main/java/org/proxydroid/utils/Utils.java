...
//  out += str(execShell(self._adb+" shell 'mount /data/local/tmp/cacerts /system/etc/security/cacerts'"))
  ...
  public static String pushCerts(){
    if(! Utils.isRoot()) {
      return "no root";
    }
    ...
    String systemCt = "";
    StringBuilder sb = new StringBuilder();
    String command = "ls -Z /system/etc/security/cacerts | head -n1\nexit\n";
    String lines = null;
    String out = "";
    runScript(command, sb, 10 * 1000, true);
//    if (exitcode == TIME_OUT) {
//      return "timeout";
//    }
    lines = sb.toString();sb.setLength(0);out += lines;
    String [] tmp = lines.split(" ");
    for(int i=0;i<tmp.length;i++){
      if(tmp[i].indexOf("u:") == 0){
        systemCt = tmp[i];
        break;
      }
    }
    ...
    //获取/data/local/tmp/目录下所有证书
    command = "ls /data/local/tmp/\nexit\n";
    runScript(command, sb, 10 * 1000, true);
    lines = sb.toString();sb.setLength(0);out += lines;
    tmp = lines.split("\n");

    // umount单独执行, redmi等会返回error
    command = "umount /system/etc/security/cacerts\nexit\n";
    runScript(command, sb, 10 * 1000, true);
    lines = sb.toString();sb.setLength(0);out += lines;

    command = "cp -pR /system/etc/security/cacerts /data/local/tmp/\n";
    for(int i=0;i<tmp.length;i++){
      if(tmp[i].endsWith(".0")){
        command += "cp /data/local/tmp/"+tmp[i]+" /data/local/tmp/cacerts/\n";
      }
    }
    command += "chmod -R 755 /data/local/tmp/cacerts\n";
    command += "chcon -R "+systemCt+" /data/local/tmp/cacerts\n";
    command += "mount /data/local/tmp/cacerts /system/etc/security/cacerts\n";
    command += "exit\n";
    ...
    runScript(command, sb, 10 * 1000, true);
    lines = sb.toString();sb.setLength(0);out += lines;
    if(lines.contains("/system/etc/security/cacerts")){
      return "success";
    }
    return out;
  }
